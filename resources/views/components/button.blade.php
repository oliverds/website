...
switch ($color) {
    ...
    case 'primary':
    default:
        $colorClasses = 'border-transparent text-white bg-purple-600 hover:bg-purple-700';
        $shadow = true;
        break;
}
@endphp
...
<span class="inline-flex rounded {{ $shadow ? 'shadow-sm' : null}}">
    <{{ $hyperscript }} {{ $attributes->merge(['type' => $href ? null : 'submit', 'href' => $href, 'class' => "inline-flex items-center px-3 py-2 border text-sm leading-4 font-medium rounded transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 $colorClasses"]) }}>
        {{ $slot }}
    </{{ $hyperscript }}>
</span>
