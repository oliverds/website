...
<feed xml:lang="es-MX" xmlns="http://www.w3.org/2005/Atom">
    ...
    <updated>{{ Carbon\Carbon::parse($posts->first()['updated_at'])->format('Y-m-d\TH:i:s\Z') }}</updated>
    @foreach ($posts as $post)
    <entry>
        <id>tag:{{ request()->getHost() }},2005:World::Post/{{ $post['id'] }}</id>
        <published>{{ Carbon\Carbon::parse($post['published_at'])->format('Y-m-d\TH:i:s\Z') }}</published>
        <updated>{{ Carbon\Carbon::parse($post['updated_at'])->format('Y-m-d\TH:i:s\Z') }}</updated>
        <link rel="alternate" type="text/html" href="{{ route('es.posts.show', ['slug' => $post['slug']]) }}" />
